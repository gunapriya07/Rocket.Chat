...
const manualRegisterResponseSchema = ajv.compile({
	type: 'object',
	properties: { success: { type: 'boolean', enum: [true] } },
	required: ['success'],
	additionalProperties: false,
...
const createRegistrationIntentResponseSchema = ajv.compile({
	type: 'object',
	properties: {
		intentData: { type: 'object' },
...
const registerPreIntentResponseSchema = ajv.compile({
	type: 'object',
	properties: {
...
const confirmationPollResponseSchema = ajv.compile({
	type: 'object',
	properties: {
...
const registrationStatusResponseSchema = ajv.compile({
	type: 'object',
	properties: {
		registrationStatus: { type: 'object' },
		success: { type: 'boolean', enum: [true] },
	},
...
const checkoutUrlResponseSchema = ajv.compile({
	type: 'object',
	properties: {
		url: { type: 'string' },
		success: { type: 'boolean', enum: [true] },
...
API.v1.post(
	...
	async function action() {
		...
		if (intentData) {
			return API.v1.success({ success: true, intentData });
		}
...
API.v1.post(
	...
	async function action() {
		return API.v1.success({ success: true, offline: !(await registerPreIntentWorkspaceWizard()) });
	},
);
...
API.v1.get(
	...
	async function action() {
		...
		if (pollData) {
			...
			return API.v1.success({ success: true, pollData });
		}
...
API.v1.get(
	...
	async function action() {
		...
		return API.v1.success({ success: true, registrationStatus });
	},
);
...
API.v1.get(
	...
	async function action() {
		...
		return API.v1.success({ success: true, url: checkoutUrl.url });
	},
);
